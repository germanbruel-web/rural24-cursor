...
export function useAdData(adId: string) {
  ...
  const loadAd = async () => {
    try {
      ...
      const [subcatR, opTypeR, sellerR] = await Promise.allSettled([
        data.subcategory_id
      ...
      const subcatResult = subcatR.status === 'fulfilled' ? subcatR.value : { data: null };
      const opTypeResult = opTypeR.status === 'fulfilled' ? opTypeR.value : { data: null };
      const sellerResult = sellerR.status === 'fulfilled' ? sellerR.value : { data: null };

      let categoryData = null;
      let parentSubcatData: { id: string; display_name: string } | null = null;

      const [catR, parentSubcatR] = await Promise.allSettled([
      ...
      categoryData = catR.status === 'fulfilled' ? catR.value.data : null;
      parentSubcatData = parentSubcatR.status === 'fulfilled' ? parentSubcatR.value.data : null;
