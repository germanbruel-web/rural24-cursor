...
// ✅ GENERADOR 100% GRATIS - Sin APIs externas
console.log("✅ Generador GRATIS activado - Base de datos de modelos REALES");
...
// 📚 Base de datos de modelos REALES del mercado
...
function getRandomModels(brandName: string, count: number): string[] {
  ...
  // Mezclar y tomar N modelos aleatorios
  const shuffled = [...models].sort(() => 0.5 - Math.random());
  return shuffled.slice(0, Math.min(count, models.length));
}

export function generateRealisticSpecs(modelName: string, subcategoryName: string): any {
  // Extraer HP del nombre del modelo
  ...
  return {
    motor: {
      tipo: hp > 150 ? 'Diésel Turbo Common Rail' : hp > 100 ? 'Diésel Turbo' : 'Diésel',
      potencia: `${hp} HP`,
    ...
    caracteristicas_especiales: [
      ...
      hp > 150 ? 'Sistema GPS integrado' : 'Pre-instalación GPS',
      hp > 100 ? 'Suspensión neumática cabina' : 'Suspensión mecánica',
      'Sistema hidráulico avanzado',
      'Toma de fuerza independiente',
      hp > 120 ? 'Tracción 4x4 con diferencial bloqueante' : 'Tracción 4x4'
    ]
  };
}
...
/**
 * ✅ Generar modelos automáticamente - 100% GRATIS
 * Usa base de datos local de modelos REALES
 */
export async function generateModelsFromAI(
  ...
  console.log(`🤖 Generando ${count} modelos REALES de ${brandName} - ${subcategoryName}...`);
  console.log(`✅ GRATIS - Base de datos local de modelos del mercado`);
  
  const realModelNames = getRandomModels(brandName, count);
  
  if (realModelNames.length === 0) {
    console.warn(`⚠️ No hay modelos precargados para ${brandName}, generando genéricos...`);
    // Fallback a modelos genéricos si no está en la base de datos
  ...
  console.log(`📊 Modelos REALES encontrados:`, realModelNames.join(', '));
  return generateModelsFromNames(brandName, subcategoryName, realModelNames);
}

/**
 * Extraer modelos desde URL (deshabilitado - usa generateModelsFromAI)
 */
export async function generateModelsFromURL(
  ...
  console.log(`ℹ️ Usando base de datos local en su lugar`);
  return generateModelsFromAI(brandName, subcategoryName, 10);
}
...
export async function insertGeneratedModels(
  ...
  console.log(`📥 Insertando ${models.length} modelos en Supabase...`);
  ...
  for (const model of models) {
    try {
      const dataToInsert = {
        ...
        verified: false,
        ai_generated: true,
        ai_confidence: model.ai_metadata?.confidence || 0.95,
        ai_source: model.ai_metadata?.source || 'Base de datos de modelos reales'
      };
      
      const { data, error } = await supabase.from('models').insert(dataToInsert).select();

      if (error) {
        console.error(`❌ Error insertando ${model.display_name}:`, error.message);
        errors++;
      } else {
        console.log(`✅ Insertado: ${model.display_name}`);
        success++;
      }
    } catch (err) {
      console.error(`❌ Error catch ${model.display_name}:`, err);
      errors++;
    }
  }
