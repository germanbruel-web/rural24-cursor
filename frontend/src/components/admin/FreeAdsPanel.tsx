...
import { Edit, Trash2, Eye, EyeOff, Star, StarOff, Zap } from 'lucide-react';
import { notify } from '../../utils/notifications';

export default function FreeAdsPanel() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const adsData = await getAllAdsByRole({ userRole: 'free' });
      console.log('📋 Avisos FREE cargados:', adsData.length);
      setAds(adsData);
    } catch (error) {
      console.error('Error loading free ads:', error);
      notify.error('Error al cargar avisos FREE');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConvertToHybrid = async (id: string) => {
    if (!confirm('¿Convertir este aviso FREE a Premium (Híbrido)?')) return;
    
    try {
      const { error } = await updateAdToHybrid(id);
      if (error) throw new Error(error);
      notify.success('Aviso convertido a Premium (Híbrido)');
      loadData();
    } catch (error: any) {
      notify.error(error.message || 'Error al convertir aviso');
    }
  };
    ...
    return (
      <div className="flex items-center justify-center py-12">
        <div className="text-center">
          ...
          <p className="text-gray-600">Cargando avisos FREE...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-[#1b2f23]">AVISOS FREE</h2>
        <p className="text-sm text-gray-600 mt-1">
          Gestión de avisos de usuarios gratuitos ({filteredAds.length} avisos)
        </p>
      </div>
      ...
      {filteredAds.length === 0 ? (
        <div className="bg-white rounded-lg shadow-sm p-12 text-center">
          <p className="text-gray-500">No hay avisos FREE {categoryFilter !== 'all' && `en la categoría ${categoryFilter}`}</p>
        </div>
      ) : (
        <div className="space-y-4">
          {filteredAds.map((ad) => (
            <div key={ad.id} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        title={ad.featured ? 'Quitar destacado' : 'Marcar como destacado'}
                      >
                      ...
                      <button
                        onClick={() => handleConvertToHybrid(ad.id)}
                        className="p-2 text-purple-600 hover:bg-purple-50 rounded-lg transition-colors"
                        title="Convertir a Premium (Híbrido)"
                      >
