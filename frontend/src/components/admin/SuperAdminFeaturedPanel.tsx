  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Administración completa de avisos destacados
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="p-6">
        ...
        {activeTab === 'manual' && <ManualActivationTab />}
      </div>
    </div>
  );
}
                  ...
                  <tr key={ad.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                      <div className="flex items-center justify-end gap-2">
                        {/* Ver Aviso */}
                        <button
                          onClick={() => window.open(`/avisos/${ad.ad_slug}`, '_blank')}
...
function CalendarTab() {
  ...
  const [selectedPlacement, setSelectedPlacement] = useState<FeaturedPlacement>('homepage');
  const [gridData, setGridData] = useState<OccupancyGridDay[]>([]);
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    loadGrid();
  }, [selectedMonth, selectedPlacement]);

  const loadGrid = async () => {
    setLoading(true);
    const year = selectedMonth.getFullYear();
    const month = selectedMonth.getMonth() + 1;
    const { data } = await getOccupancyGrid(year, month, selectedPlacement);
    setGridData(data);
    setLoading(false);
  };
  ...
  const monthName = selectedMonth.toLocaleDateString('es-AR', { month: 'long', year: 'numeric' });

  // Agrupar por categoría
  const categories = Array.from(new Set(gridData.map(d => d.category_id)));
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          {categories.map(categoryId => {
            const categoryData = gridData.filter(d => d.category_id === categoryId);
            const categoryName = categoryData[0]?.category_name || 'Sin categoría';
            
            return (
              <div key={categoryId} className="border border-gray-200 rounded-lg p-4">
                <h4 className="font-semibold text-gray-900 mb-4">{categoryName}</h4>
                <div className="grid grid-cols-7 gap-2">
                  ...
                  {categoryData.map(dayData => {
                    const occupancy = dayData.count_active / dayData.max_slots;
                    const bgColor =
                      occupancy === 0
                    ...
                    return (
                      <div
                        key={dayData.date}
                        className={`border rounded p-2 text-center ${bgColor}`}
                        title={`${dayData.date}: ${dayData.count_active}/${dayData.max_slots} slots`}
                      >
                        <div className="text-sm font-medium">{dayNumber}</div>
                        <div className="text-xs text-gray-600 mt-1">
                          {dayData.count_active}/{dayData.max_slots}
                        </div>
                      </div>
                    );
                  })}
