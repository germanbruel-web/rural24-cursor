...
export async function POST(request: NextRequest) {
  return withAuth(request, async (_user: AuthUser) => {
    ...
    const { data, error } = await supabase
      .from('onboarding_slides')
      .insert({ title: title.trim(), description: description || null, image_url: image_url || null, sort_order: sort_order ?? 0, is_active: is_active ?? true, target_device: target_device ?? 'both', bg_color: bg_color ?? '#14532d', image_fit: image_fit ?? 'cover' })
      .select()
      .single();
