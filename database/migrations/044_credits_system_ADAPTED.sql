-- ============================================
-- MIGRACIÓN ADAPTADA: Sistema de Créditos y Destacados
-- Fecha: 11 Febrero 2026
-- ADAPTADO AL SCHEMA REAL DE RURAL24
-- ============================================
...
-- ============================================
-- 2. ACTUALIZAR: subscription_plans (tabla real)
-- ============================================
DO $$
BEGIN
  -- Agregar columna slug si no existe (necesaria para lógica de planos)
  IF NOT EXISTS (
    ...
    -- Generar slugs basados en display_name
    UPDATE public.subscription_plans
    SET slug = LOWER(REPLACE(name, ' ', '-'))
    WHERE slug IS NULL;
  END IF;
  
  -- Agregar columnas de créditos
  ALTER TABLE public.subscription_plans
  ...
  -- Actualizar planes existentes con créditos mensuales
  ...
  RAISE NOTICE 'subscription_plans actualizado con sistema de créditos';
END $$;
...
CREATE TABLE IF NOT EXISTS public.credit_transactions (
  ...
  payment_id UUID,
  featured_ad_id UUID, -- Forward reference, se crea después
...
-- Agregar foreign key de credit_transactions a featured_ads
...
BEGIN
  ...
  -- 4. Verificar aviso (ajustado para status real de la tabla ads)
  ...
  -- 10. Actualizar la transacción con el featured_ad_id
...
-- ============================================
-- 11. FUNCIONES AUXILIARES (Queries comunes)
-- ============================================

-- Obtener featured ads por categoría
...
-- Obtener featured ads del usuario
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migración 044 ADAPTADA completada exitosamente';
  RAISE NOTICE '✅ Tablas creadas: global_config, user_credits, credit_transactions, featured_ads';
  RAISE NOTICE '✅ subscription_plans actualizado con columnas de créditos';
  RAISE NOTICE '✅ 10 funciones RPC creadas';
